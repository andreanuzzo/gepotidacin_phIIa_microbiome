...
Fig1b <- map(levels(div.df$Type),
             ~div.df %>% 
               filter(Type==.x) %>% 
                ...
                stat_compare_means(paired=T, method = 'kruskal', label.y.npc = 'bottom', label = 'p.format')+
...
### Fig2b
```{r}
Fig2b <- phyloseq::distance(pslog, method = 'wunifrac', type = 'PCoA') %>% 
  as.matrix %>% 
  as.data.frame %>% 
  rownames_to_column %>% 
  ...
  stat_compare_means(aes(label = ..p.signif..), 
                     comparisons = list(c('Day 1 : Day 1', 'Day 1 : Day 5'),
                                        c('Day 1 : Day 1', 'Day 1 : Follow-up')),
...
Fig2c <- wrap_plots(ordplots.pcoa.nmds, ncol=3)+
  plot_layout(guides='collect') &
  theme(legend.position='bottom')
```
...
### Fig2d
```{r}
Fig2d <- phyloseq::distance(pslog, method = 'wunifrac', type = 'NMDS') %>% 
  as.matrix %>% 
  as.data.frame %>% 
  rownames_to_column %>% 
  pivot_longer(-rowname) %>% 
...
FigS1b <- scores(cca_model, display = 'sites') %>% 
  as.data.frame %>% 
  bind_cols(data.frame(sample_data(phylobj))) %>% 
...
Fig3 <- map(levels(original_metadata$Type),
      ~ reslist %>% 
...
# Save figures
```{r}
ggarrange(Fig1, Fig1b, labels = 'AUTO', ncol=1) %>% 
